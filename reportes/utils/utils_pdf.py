...
from .utils_azure import generate_url_sas
...
class GeneradorReportePDF:
    ...
    def _preload_logo(self) -> Optional[ImageReader]:
        try:
            logo_path = os.path.join(settings.BASE_DIR, 'reportes/static/images/logo.png')
    ...
    def _load_image(self, pieza: Any) -> Optional[ImageReader]:
        if not pieza.imagen:
            return None
        try:
            url_sas = generate_url_sas(pieza.imagen.name, expira_en_min=5)
    ...
    def _dibujar_paginacion(self):
        y_pag = 20
        texto = f'REPORTE N° {self.reporte.id}  |  Página {self.page_num}'
    ...
    def dibujar_encabezado(self, es_primera_pagina: bool = False):
        ...
        self._text(self.ml + w1 + w2 + w3 / 2,  y - 27, f'REPORTE N° {self.reporte.id}', 'Helvetica-Bold', 11, ROJO, 'center')

        self.y_curr -= 35

        self._rect_fill(self.ml, self.y_curr - 26, ancho, 26, NEGRO)
        self._text(self.ml + ancho / 2, self.y_curr - 17, 'RECHAZO DE MATERIALES', 'Helvetica-Bold', 14, BLANCO, 'center')
        self.y_curr -= 26
        ...
        total = self.reporte.piezas_rechazadas.count()
    ...
    def _dibujar_meta(self):
        ...
        self._text(bx + bw / 2, y - 16, 'REPORTE', 'Helvetica-Bold', 7, BLANCO, 'center')
        self._text(bx + bw / 2, y - 42, str(self.reporte.id), 'Helvetica-Bold', 26, BLANCO, 'center')

        campos = [
            ('Remito Recepción', self.reporte.remito_recepcion or 'N/A'),
            ('Fecha',            self.reporte.fecha.strftime('%d/%m/%Y')),
            ('Cliente',          self.reporte.cliente.nombre.title()),
            ('Empleado a Cargo', f'{self.reporte.empleado.nombre} {self.reporte.empleado.apellido}'.title()),
        ]
        col_w = (ancho - bw - 16) / 2
        row_h = 32 
    ...
    def dibujar_pieza(self, pieza: Any, numero: int, es_ultima: bool = False):
        ...
        dano_txt = str(pieza.categoria_dano)
        dano_w = stringWidth(dano_txt, 'Helvetica-Bold', 9) + 20
        dano_x = self.mr - dano_w - 8
        self._rect_fill(dano_x, y_top - 23, dano_w, 18, ROJO_LIGHT, '#ffbbbb', radius=8)
        self._text(dano_x + dano_w / 2, y_top - 15, dano_txt, 'Helvetica-Bold', 9, '#c0392b', 'center')
        ...
        self._text(x_txt + 70, cy, 'Tipo / Medida', 'Helvetica-Bold', 9, GRIS_OSCURO)
        cy -= 18
        
        self._text(x_txt, cy, str(pieza.cantidad), 'Helvetica-Bold', 18, ROJO)
        tipo_med = f'{pieza.pieza.categoria.descripcion} — {pieza.pieza.medidas}'
    ...
    # --- Pie de Pagina, Fijo al final de la pagina ---
    def dibujar_pie_final(self):
        ...
        self._rect_fill(self.ml, y_pie + 47, ancho, 18, NEGRO, radius=4)
        self._rect_fill(self.ml, y_pie + 47, ancho, 9,  NEGRO)
        self._text(self.ml + 12, y_pie + 52, 'INFORMACIÓN DEL TRANSPORTE', 'Helvetica-Bold', 9, BLANCO)

        if self.reporte.transportista:
            tr = self.reporte.transportista
            mitad = self.ml + ancho / 2

            self._text(self.ml + 12, y_pie + 32, 'Vehículo', 'Helvetica-Bold', 9, GRIS_MED)
            self._text(self.ml + 65, y_pie + 32, tr.get_transporte_display(), 'Helvetica', 10, NEGRO)
            self._text(mitad,      y_pie + 32, 'Patente', 'Helvetica-Bold', 9, GRIS_MED)
            self._text(mitad + 55, y_pie + 32, self.reporte.patente_reporte, 'Helvetica', 10, NEGRO)
            self._text(self.ml + 12, y_pie + 14, 'Chofer', 'Helvetica-Bold', 9, GRIS_MED)
            self._text(self.ml + 65, y_pie + 14, f'{tr.nombre.title()} {tr.apellido.title()}', 'Helvetica', 10, NEGRO)
        ...
        if self.reporte.transportista:
            tr = self.reporte.transportista
            self._text(self.ml + 16 + firma_w / 2, y_firma - 24, f'{tr.nombre.title()} {tr.apellido.title()}', 'Helvetica', 9, GRIS_MED, 'center')
        ...
        emp = self.reporte.empleado
        self._text(x_f2 + firma_w / 2, y_firma - 24, f'{emp.nombre} {emp.apellido}'.title(), 'Helvetica', 9, GRIS_MED, 'center')
    ...
    def generar(self) -> bytes:
        ...
        piezas = list(self.reporte.piezas_rechazadas.all())
